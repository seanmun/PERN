    ...
    <article className="mx-auto max-w-4xl px-4 py-16 sm:py-20">
      ...
      <Section id="routes" title="6. Routes — every page">
        ...
        <List>
          ...
          <li><strong>HeaderAvatar</strong> ([components/HeaderAvatar.tsx](components/HeaderAvatar.tsx) → [HeaderAvatarLink.tsx](components/HeaderAvatarLink.tsx)): avatar links to <code>/me</code>, and nothing else. The Admin shortcut it used to carry is gone — with it went the two queries that resolved every trip slug the viewer could administer, plus the slug-vs-pathname check that inferred which trip you were on.</li>
          <li><strong>Editing an event</strong> is reached from the <strong>Edit</strong> button on <code>/trips/[slug]/schedule</code>, shown only when <code>canEditTrip(ctx, trip.id)</code> is true. It links to <code>/trips/[slug]/edit</code> — the round-builder in edit mode, which is the whole of admin now. The gate sits next to the thing it gates, on a page that already knows which trip is in scope.</li>
